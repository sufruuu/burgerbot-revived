...
def generate_launch_description():
    ...
    pkg_bcr = get_package_share_directory('burgerbot_rev1')
    ...
    slam_toolbox_launch_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_slam_toolbox_dir, 'launch', 'offline_launch.py')
        ),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'slam_params_file': os.path.join(pkg_bcr, 'config', 'mapper_params_offline.yaml'),
        }.items()
    )
